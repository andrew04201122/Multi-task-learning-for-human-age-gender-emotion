...
def relu(x, leakiness=0.00):
    return tf.where(tf.less(x, 0.0), leakiness * x, x+0.0, name='leaky_relu')


def FC(name, x, output_dim, keep_rate, activation='relu'):
    assert (activation == 'relu') or (activation == 'softmax') or (activation == 'linear') or (activation == 'sigmoid')
    with tf.compat.v1.variable_scope(name):
        ...
        if activation != 'relu':
            return x      #final layer not use dropout
        else:
